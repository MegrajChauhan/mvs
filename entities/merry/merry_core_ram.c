...
#define _MERRY_MEM_PAGE_ACCESS_(mem, addr) (MerryCoreRAMPage*)mvs_dynamic_listl_ref_of_unsafe((mem)->pg_list, (addr) / _MERRY_CORE_PAGE_LEN_IN_BYTES_)
...
mbool_t merry_core_memory_populate(MerryCoreRAM *mem, msize_t space_len,
  ...
  msize_t pg_count =
      ...
      _MERRY_CORE_PAGE_LEN_IN_BYTES_; // space_len should be aligned most of the
                                      // time unless instruction len is extremely
                                      // small
  ...
  if (space_len % _MERRY_CORE_PAGE_LEN_IN_BYTES_) {
    ...
    MVSMappedMemory *pg;
	if (mvs_mapped_memory_create(&pg, MVS_INTERFACE_CONF_SHAREABLE) != MRES_SUCCESS) {
		MERRY_ERR("Failed to populate RAM");
		mvs_dynamic_listl_destroy(mem->pg_list);
		return mfalse;
	}
    if (mvs_mapped_memory_map(pg, _MERRY_CORE_PAGE_LEN_IN_BYTES_) != MRES_SUCCESS) {
		MERRY_ERR("Failed to populate RAM");
		mvs_dynamic_listl_destroy(mem->pg_list);
    ...
    memcpy(page.repr.bytes, addr_space + (pg_count) * _MERRY_CORE_PAGE_LEN_IN_BYTES_, (space_len % _MERRY_CORE_PAGE_LEN_IN_BYTES_));
...
mbool_t merry_core_memory_read_byte(MerryCoreRAM *mem, maddress_t addr,
  ...
  if (surelyF(addr >= mem->max_address))
    {mem->err = MEM_ERR_OUT_OF_BOUNDS_ACCESS; return mfalse;}
  *store_in = (_MERRY_MEM_PAGE_ACCESS_(mem, addr))->repr.bytes[addr % _MERRY_CORE_PAGE_LEN_IN_BYTES_];
  return mtrue;
}
...
mbool_t merry_core_memory_read_word(MerryCoreRAM *mem, maddress_t addr,
  ...
  if (surelyF(addr >= mem->max_address))
    {mem->err = MEM_ERR_OUT_OF_BOUNDS_ACCESS; return mfalse;}
  *store_in =
      *(mwptr_t)&(_MERRY_MEM_PAGE_ACCESS_(mem, addr))->repr.bytes[addr % _MERRY_CORE_PAGE_LEN_IN_BYTES_];
  return mtrue;
}
...
mbool_t merry_core_memory_read_byte_atm(MerryCoreRAM *mem, maddress_t addr,
  ...
  *store_in = atomic_load_explicit(
      (_Atomic mbyte_t
           *)(&(_MERRY_MEM_PAGE_ACCESS_(mem, addr))->
                   repr.bytes[addr % _MERRY_CORE_PAGE_LEN_IN_BYTES_]),
...
mbool_t merry_core_memory_read_bulk(MerryCoreRAM *mem, maddress_t addr,
  ...
  for (msize_t i = pg; i <= end_page; i++) {
    memcpy(iter, (_MERRY_MEM_PAGE_ACCESS_(mem, addr))->repr.bytes + off, to_read_this_page);
...
mbool_t merry_core_memory_write_dword_atm(MerryCoreRAM *mem, maddress_t addr,
  ...
  if (surelyF(addr >= mem->max_address))
    {mem->err = MEM_ERR_OUT_OF_BOUNDS_ACCESS; return mfalse;}
  atomic_store_explicit((_Atomic mdword_t *)&(_MERRY_MEM_PAGE_ACCESS_(mem, addr))->
                            repr.bytes[addr % _MERRY_CORE_PAGE_LEN_IN_BYTES_],
                        store, memory_order_release);
  return mtrue;
}
...
mbool_t merry_core_memory_write_qword_atm(MerryCoreRAM *mem, maddress_t addr,
  ...
  if (surelyF(addr >= mem->max_address))
    {mem->err = MEM_ERR_OUT_OF_BOUNDS_ACCESS; return mfalse;}
  atomic_store_explicit((_Atomic mqword_t *)&(_MERRY_MEM_PAGE_ACCESS_(mem, addr))->
                            repr.bytes[addr % _MERRY_CORE_PAGE_LEN_IN_BYTES_],
                        store, memory_order_release);
  return mtrue;
}
...
mbool_t merry_core_memory_write_bulk(MerryCoreRAM *mem, maddress_t addr,
  ...
  for (msize_t i = pg; i <= end_page; i++) {
    memcpy((_MERRY_MEM_PAGE_ACCESS_(mem, addr))->repr.bytes + off, iter, to_write_this_page);
...
void merry_core_memory_destroy(MerryCoreRAM *mem) {
  ...
  if (mem->pg_list) {
     for (msize_t i = 0; i < mvs_dynamic_listl_size_unsafe(mem->pg_list); i++) {
		MerryCoreRAMPage *pg = (MerryCoreRAMPage*)mvs_dynamic_listl_ref_of_unsafe(mem->pg_list, i);
