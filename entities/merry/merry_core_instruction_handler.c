...
merry_core_ihdlr(div_imm) {
  ...
  if (op2 == 0) {
    MERRY_ERR("Div by Zero: PC=%zu", core->PC);
    return mfalse;
  }
  core->REGISTER_FILE[op1] /= op2;
  core->flags.regr = mvs_obtain_flags_regr();
...
merry_core_ihdlr(loadsq) {
  ...
  if (res >= core->SP) {
    MERRY_ERR("Invalid STACK Access: PC=%zu", core->PC);
    return mfalse;
  }
