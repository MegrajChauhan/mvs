...
MerryCoreRAM *merry_core_memory_init() {
  ...
  if (!mem) {
    MERR("Failed to allocate memory for Core RAM");
    return RET_NULL;
  }
  mem->pg_list = NULL;
  mem->max_address = 0;
...
mbool_t merry_core_memory_populate(MerryCoreRAM *mem, msize_t space_len,
  ...
  if (res != mtrue) {
    MERR("RAM Page Buffer allocation failed[PGC=%zu]", pg_count);
    return mfalse;
  }
...
mbool_t merry_core_memory_read_byte(MerryCoreRAM *mem, maddress_t addr,
  ...
  if (surelyF(addr >= mem->max_address)) {
  	MERR("Out of Bounds Access[addr=%zu]", addr);
    return mfalse;
  }
...
mbool_t merry_core_memory_read_word(MerryCoreRAM *mem, maddress_t addr,
  ...
  if (surelyF((addr % 2) != 0)) {
    MERR("Misaligned memory access[addr=%zu]", addr);
    return mfalse; 
  }
  *store_in =
