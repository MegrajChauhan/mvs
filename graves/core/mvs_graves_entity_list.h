...
struct MVSGravesEntityList {
  ...
  msize_t
      total_entity_count; // The total number of entities that the list can have
  msize_t current_entity_count; // How many entities the list currently has,
                                // active and dead? Used to provide ID
  atm_msize_t
      total_entities_ever_created; // accounting information, used for UID
...
_MVS_ATTR_ALWAYS_INLINE_ mbool_t mvs_graves_entity_list_validate_identity(
    MVSGravesEntityList *list, MVSEntityIdentity *iden) {
