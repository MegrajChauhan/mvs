...
_MVS_ATTR_INTERNAL_ mbool_t mvs_graves_initialize_self() {
   ...
   if (mvs_dynamic_listl_create(&graves.entity_list, 10, sizeof(MVSEntity*)) != MRES_SUCCESS)
		   return mfalse;
...
/*----Utility Functions----*/
_MVS_ATTR_INTERNAL_ mthreadRet_t mvs_graves_entity_launcher_for_command_execution(mptr_t ent) {
		// This is a special function just for the entities that were launched by the launch
		// commands if -es flag was provided
...
_MVS_ATTR_INTERNAL_ msize_t mvs_graves_execute_launch_commands() {
		...
		mvs_log_dbg("Launch commands given: %zu commands", graves.cmd_opts->entities_to_spawn);
