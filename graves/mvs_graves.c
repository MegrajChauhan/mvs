...
/*----Request Handlers----*/
_MVS_ATTR_INTERNAL_ msize_t mvs_request_handle_SPAWN_ENTITY(MVSGravesRequest* req);
...
_MVS_ATTR_INTERNAL_ mbool_t mvs_graves_check_config_valid(msize_t ID, mqword_t conf);
_MVS_ATTR_INTERNAL_ mbool_t mvs_graves_check_properties_valid(msize_t ID, mqword_t prop);
_MVS_ATTR_INTERNAL_ MVSEntity* mvs_graves_find_free_entity();
_MVS_ATTR_INTERNAL_ MVSEntity* mvs_graves_create_new_entity();
_MVS_ATTR_INTERNAL_ void mvs_graves_initialize_new_entity(MVSEntity* entity, msize_t ID, mqword_t conf, mqword_t prop);
...
_MVS_ATTR_INTERNAL_ msize_t mvs_graves_init(MVSArgParseResult *opts) {
    ...
    if ((res = mvs_dynamic_listl_create(&graves.entity_list,_MVS_CONSTANT_ENTITY_LIST_INITIAL_LEN_, sizeof(MVSEntity*))) != MRES_SUCCESS) {
       mvs_cond_destroy(&graves.graves_cond);
...
_MVS_ATTR_INTERNAL_ msize_t mvs_graves_run() {
	 ...
	 * 2. (optional/in-future)if no entities were requested, provide a command interpreter
	 ...
	 */
     ...
     while (mtrue) {
     	...
     	if ((req = mvs_request_queue_manager_dequeue_request(graves.req_queue)) == NULL) {
     		// probably empty
     		mvs_cond_wait(&graves.graves_cond, &graves.graves_lock);
     	} else {
           MVSEntity *entity = mvs_graves_get_entity(req->iden);
           if (!entity) {
           	   mvs_log_err("Invalid Request Made: ID=%zu, UID=%zu", req->iden->ID, req->iden->UID);
               req->RESULT[0] = _MVS_CONSTANT_REQUEST_REQ_INVALID_;
           } else {
           	   // handle the request
    		   if (req->type >= MREQ_COUNT) {
           	   	mvs_log_err("Invalid Request: ID=%zu, UID=%zu, REQ=%zu", req->iden->ID, req->iden->UID, req->type);
				...
				req->RESULT[1] = res;
    		   	mvs_log_dbg("Request Served:ID=%zu, UID=%zu, REQ=%zu(RET=%zu)", req->iden->ID, req->iden->UID, req->type, res);
    		   }
           }
...
_MVS_ATTR_INTERNAL_ MVSEntity* mvs_graves_get_entity(MVSEntityIdentity *iden) {
	...
	MVSEntity *entity;
	if (mvs_dynamic_listl_ref_of(graves.entity_list, &entity, iden->ID) != MRES_SUCCESS) 
	   return NULL; // this shall never be executed
	return entity;
}

void mvs_run(MVSArgParseResult *opts) {
   if (mvs_graves_launch_logger(MLOG_DBG/*for now*/) != 0) {
...
msize_t mvs_graves_API_make_request(MVSEntityIdentity *hdlr, MVSGravesRequest *req) {
    if (!hdlr || !req)
      return 2;
	mvs_mutex_lock(&graves.graves_lock);
	msize_t ret = 0;
    /*
     * I realize that it would be a smart move to verify hdlr and req here instead of the
     * serve loop but this is supposed to be a quick call.
     */
    /*
     * hdlr isn't really important here. In future, it could help keep accounting information.
     */
    if (req->wakeup_cond) {
    	...
    	if (mvs_request_queue_manager_enqueue_request_async(graves.req_queue, req) != 0)
    	   ret = 0;
    	else 
    	   ret = 1;    	
    }
...
/*
 * The reason that there is an ID here is because some configs and properties might be entity specific
 * This allows me to make sure to check it in the future.
 * Or even provide a simple request to check the capability of an entity
 * Thus, it can also be said that there are MVS defined configs of an entity but the
 * entity can be configured as well.
 * And users can also use the command line to disable or enable some configuration
 */
_MVS_ATTR_INTERNAL_ mbool_t mvs_graves_check_config_valid(msize_t ID, mqword_t conf) {
...
_MVS_ATTR_INTERNAL_ MVSEntity* mvs_graves_find_free_entity() {
	 ...
	 * entities but it wouldn't be genius to have a separate list to store the IDs of
	 * free entities either. The number of entities heavily influences the performance.
	 * For high number of entities, keeping a cache like that may prove to be worth it
	 * but it increases the chances of fragmentation too
	 */
...
_MVS_ATTR_INTERNAL_ void mvs_graves_initialize_new_entity(MVSEntity* entity, msize_t ID, mqword_t conf, mqword_t props) {
	entity->type = MENTITY_LOCAL;
	entity->EID = ID;
	atomic_store_explicit(&entity->state.state, MENTITY_STOPPED, memory_order_relaxed);
...
_MVS_ATTR_INTERNAL_ mbool_t mvs_graves_launch_new_entity(MVSEntity *entity) {
    ...
    mbptr_t entity_repr;
	if (entry->create(&context, &entity_repr) != 0) {
		return mfalse;
	}
	entity->entity_repr = (mptr_t)entity_repr;
	mthread_t th;
	if (mvs_create_detached_thread(&th, mvs_graves_entity_launcher, entity) != MRES_SUCCESS) {
...
_MVS_ATTR_INTERNAL_ mthreadRet_t mvs_graves_entity_launcher(mptr_t ent) {
    ...
    MVSEntityRegistryEntry	*entry = mvs_registry_get_entry(entity->EID);
	atomic_store_explicit(&entity->state.state, MENTITY_RUNNING, memory_order_release);
	while (mtrue) {
		switch (entry->exec(entity->entity_repr)) {
			case 0:
				goto __mvs_graves_terminate_entity;
		    case 2: {
		    	atomic_store_explicit(&entity->state.state, MENTITY_BUSY, memory_order_release);
                break;
		    }
		    default:
		    	atomic_store_explicit(&entity->state.state, MENTITY_RUNNING, memory_order_release);
...
__mvs_graves_terminate_entity:
   ...
   if (!graves.active_entities)
     mvs_cond_signal(&graves.graves_cond);
   mvs_mutex_unlock(&graves.graves_lock);
   mvs_log_dbg("Entity[ID=%zu,UID=%zu] exited with code %zu", entity->identity.ID, entity->identity.UID, ret);
   return NULL;
}
...
_MVS_ATTR_INTERNAL_ msize_t mvs_request_handle_SPAWN_ENTITY(MVSGravesRequest *req) {
     ...
     */	
     ...
     mvs_log_dbg("Serving Request[SPAWN_ENTITY]: ID=%zu, UID=%zu for EID=%zu", req->iden->ID, req->iden->UID, ID);
	...
	if (!entry) {
     ...
     if (ID >= _MVS_CONSTANT_ENTITY_COUNT_) {
        mvs_log_err("Invalid EID[%zu] provided for SPAWNING[ID=%zu, UID=%zu]",ID,req->iden->ID, req->iden->UID);
        return _MVS_CONSTANT_REQUEST_REQ_INVALID_EID_; 
     }
     if (!mvs_graves_check_config_valid(ID, req->args.spawn_entity.config)) {
        mvs_log_err("Invalid Config provided for SPAWNING[ID=%zu, UID=%zu]",req->iden->ID, req->iden->UID);
        return _MVS_CONSTANT_REQUEST_REQ_INVALID_CONFIG_;
     }
     if (!mvs_graves_check_properties_valid(ID, req->args.spawn_entity.properties)) {
        mvs_log_err("Invalid Properties provided for SPAWNING[ID=%zu, UID=%zu]",req->iden->ID, req->iden->UID);
     ...
     if (!entity) {
       ...
       mvs_graves_initialize_new_entity(entity, ID, req->args.spawn_entity.config, req->args.spawn_entity.properties);
     ...
     if (!mvs_graves_launch_new_entity(entity))
      ...
      mvs_log_dbg("Serving Request[SPAWN_ENTITY] Success: ID=%zu, UID=%zu for EID=%zu", req->iden->ID, req->iden->UID, ID);
      mvs_log_dbg("New Entity: ID=%zu, UID=%zu", entity->identity.ID, entity->identity.UID);
