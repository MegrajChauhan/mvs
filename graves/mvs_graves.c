 ...
 * First of all, the list file is read and all the entities are loaded into registry list.
 * Once the registry list is populated, Graves registers each entity one by one. Finally,
 * the command line options are parsed. If a spawn list is given, it is parsed. Then and 
 * only then, Graves starts self-initialization. Here, in future, configurations might be 
 * added. The logger is launched, Graves finishes initialization and all entities are launched
 * . Then Graves starts its own loop.
 * */
...
_MVS_ATTR_INTERNAL_ mbool_t mvs_graves_make_sense_of_cmd_opts() {
  ...
  if (graves.state.cmd_opts.slist) {
    ...
    if (!mvs_slist_make_commands(&graves.components.slist, graves.state.cmd_opts.slist,
...
_MVS_ATTR_INTERNAL_ mbool_t mvs_graves_init_components() {
 ...
 if (total_ecount && (graves.components.entity_list = mvs_graves_entity_list_create(total_ecount, (mqword_t)(-1))) ==
      NULL) {
    mvs_log_err("Graves entity list initialization failed");
    return mfalse;
  }
  if ((graves.components.queue_manager = mvs_request_queue_manager_create(&graves.state.config)) == NULL) {
...
_MVS_ATTR_INTERNAL_ mbool_t mvs_graves_populate_registry() {
   // After RLIST is populated from a list file, Graves will populate the registry.
   for (msize_t i = 0; i < graves.state.config.MAX_EID; i++) {
		mentityRegister_t registrar;
		mbool_t sys = mfalse;
		if (!(registrar = mvs_rlist_get_registrar(&graves.components.rlist, i, &sys))) {
		   fprintf(stderr, "[RLIST]: Couldn't obtain the registrar for EID=%zu\n", i);
		   return mfalse;
		}
		msize_t ret;
		if ((ret = registrar(i, graves.local_API)) != 0) {
		   fprintf(stderr, "[RLIST]: Registration of EID=%zu failed with return %zu", i, ret);
		   return mfalse;
		}
   }
   return mtrue;
...
_MVS_ATTR_INTERNAL_ mbool_t mvs_graves_pre_init(mstr_t *argv, msize_t argc) {
   ...
   if (!mvs_rlist_init(&graves.components.rlist))
		   return mfalse;
   if (!mvs_rlist_load_from_list_file(&graves.components.rlist, "./entities.l")) {
...
_MVS_ATTR_INTERNAL_ mbool_t mvs_graves_initialize_slist_entries() {
    MVSDynamicListLinear *commands = mvs_slist_get_commands(&graves.components.slist);
      ...
      for (msize_t i = 0; i < slist_len; i++) {
        ...
        if (cmd->local_list) {
          ...
          for (msize_t j = 0; j < mvs_dynamic_listl_size_unsafe(ll); j++) {
			msize_t ID_to_add = *(msize_t*)mvs_dynamic_listl_ref_of_unsafe(ll, j);
            MVSSlistCommand *cmd_to_add = *(
                MVSSlistCommand **)mvs_dynamic_listl_ref_of_unsafe(commands, ID_to_add);
		    mvs_log_dbg("ENTITY[%zu]: ID=%zu is running: adding to local ID=%zu", i, ID_to_add, j);
			...
			if (!mvs_graves_entity_utils_add_to_local_list(ent, ent_to_add)) {
				mvs_log_err("[UNREACHABLE]: mvs_graves_entity_utils_add_to_local_list failed mvs_graves_populate_local_list");
				// This should be unreachable code because the local list is initialized for
				// exactly the needed number of entries
				return mfalse;
			}
          }
        }
...
_MVS_ATTR_INTERNAL_ mbool_t mvs_graves_launch_entities() {
  ...
  for (msize_t i = 0; i < mvs_graves_entity_list_get_current_entity_count(graves.components.entity_list); i++) {
    mvs_log_dbg("launching ENTITY[%zu]",i);
    mthread_t th;
    MVSEntity *ent = mvs_graves_entity_list_get_entity_by_ID(graves.components.entity_list, i);
    if (mvs_create_detached_thread(&th, mvs_graves_entity_launcher_wait, ent) != MRES_SUCCESS) {
  ...
  msize_t count = mvs_graves_entity_list_get_current_entity_count(graves.components.entity_list);
  ...
  mvs_log_dbg("Active entities: %zu", mvs_graves_entity_list_get_active_entity_count(graves.components.entity_list));
  return mtrue;
}
...
_MVS_ATTR_INTERNAL_ mthreadRet_t mvs_graves_entity_launcher_wait(mptr_t e) {
  ...
  mvs_graves_entity_list_register_active_entity(graves.components.entity_list, (MVSEntity*)e);
  ...
  if (!graves.state.all_initial_entities_launched) {
    ...
    mvs_graves_entity_list_unregister_active_entity(graves.components.entity_list, (MVSEntity*)e);
    return NULL;
  }
...
__mvs_graves_entity_launcher_nowait_terminate:
  ...
  mvs_graves_entity_list_unregister_active_entity(graves.components.entity_list, ent);
  atomic_store_explicit(&ent->state, MENTITY_STOPPED, memory_order_release);
  if (mvs_graves_entity_list_get_active_entity_count(graves.components.entity_list) == 0)
    mvs_barrier_signal(&graves.sync.sleep_barrier);
  return NULL;
}
...
void mvs_run(mstr_t *argv, msize_t argc) {
  /*
   * Read the list file and populate the registry list. Then load the entities and populate
   * the entity registry. After that, parse the command line options.
   * */
   ...
   * */
  if (graves.state.cmd_opts.entities_to_spawn > 0 || graves.state.cmd_opts.slist) {
...
_MVS_ATTR_INTERNAL_ void mvs_graves_run() {
  ...
  while (mvs_graves_entity_list_get_active_entity_count(graves.components.entity_list)) {
    mvs_barrier_wait(&graves.sync.sleep_barrier);
    if (!mvs_graves_entity_list_get_active_entity_count(graves.components.entity_list))
		break;
    req = mvs_request_queue_manager_dequeue_request(graves.components.queue_manager);
    ...
    entity =
        mvs_graves_entity_list_get_entity(graves.components.entity_list, req->iden);
...
apiRes_t mvs_graves_make_request(MVSEntityIdentity *iden, GravesRequest **req) {
  ...
  if (mvs_request_queue_manager_enqueue_request(graves.components.queue_manager, *req) != 0)
