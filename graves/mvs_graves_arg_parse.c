...
_MVS_ATTR_INTERNAL_ mstr_t HELP_MSG = "Usage: mvs [options]....\n"
				  ...
				  "-es                       Ensure that all launch commands are successfully executed else terminate\n"
				  "-spawn=<N> EID <-=K>[...] Make a request to spawn 'N' instances of entity with entity ID 'EID'\n"
				  "                          Optionally provide arguments to be passed to the entity. The arguments are\n"
				  "                          are provided after '-=K' where 'K' is the number of arguments to be passed\n"
				  "-slist [command_file]     Provide a command file containing the spawn commands instead of providing them\n"
				  "                          through arguments. Using a command file allows the user to provide flags and set config\n"
				  "                          unlike -spawn where the default config and properties are set\n"
                  ;
...
mbool_t mvs_SPAWN_ENTITY_COMMAND(MVSArgParse* parser, MVSArgParseResult *res) {
  ...
  if (mvs_args_left_to_parse(parser) < 2) {
	  fprintf(stderr, "<ArgParse>: SPAWN ENTITY commands require the EID as well\n");
      return mfalse;
  }
  ...
  if (len > 6) {
    if (len > 7 && *(instance_count) == '=') {
	  ...
	  if (tmp) {
		fprintf(stderr, "<ArgParse>: Invalid value provided for INSTANCE COUNT '%s'\n", instance_count);
		return mfalse;
	  }
    } else {
  ...
  if (any_args) {
		  ...
		  if (len >= 3 && *any_args == '-' && *(any_args + 1) == '=') {
				  ...
				  if (tmp) {
						  fprintf(stderr, "<ArgParse>: Arg count for spawn command[EID=%zu, INSTANCE_COUNT=%zu] is invalid '%s'\n", EID, INSTANCE_COUNT, any_args-2);
						  return mfalse;
				  }
				  if (mvs_args_left_to_parse(parser) < ARG_COUNT) {
						  fprintf(stderr, "<ArgParse>: SPAWN ENTITY command for EID=%zu with INSTANCE_COUNT=%zu dictates %zu arguments but there isn't enough arguments provided\n", EID, INSTANCE_COUNT, ARG_COUNT);
				          return mfalse;
				  }
				  args = mvs_arg_parse_arg_slice(parser);
				  mvs_arg_parse_consume_args(parser, ARG_COUNT-1/*since the caller, mvs_parse_all_arg, advances by 1 as well*/);
		  } else {
				  ARG_COUNT = 0;
				  args = NULL;
		  }
  }
  MVSEntitySpawnCommand *command = (MVSEntitySpawnCommand *)malloc(sizeof(MVSEntitySpawnCommand));
  if (command) {
		  fprintf(stderr, "<ArgParse>: Registering ENTITY SPAWN command failed[Memory allocation failure]\n");
		  return mfalse;
  }
  command->EID = EID;
