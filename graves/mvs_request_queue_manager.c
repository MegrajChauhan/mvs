...
MVSRequestQueueManager *mvs_request_queue_manager_create() {
	...
	MVSRequestQueueManager *rman = (MVSRequestQueueManager*)malloc(sizeof(MVSRequestQueueManager));
	if (!rman) {
		mvs_log_err("Failed to initialize Request Queue Manager: Memory Allocation Failure[%s]", strerror(errno));
		return NULL;
	}
	mResult_t res;
	if ((res = mvs_dynamic_lqueue_create(&rman->request_queue, _MVS_CONSTANT_REQUEST_QUEUE_INITIAL_LENGTH_, sizeof(MVSGravesRequest*))) != MRES_SUCCESS) {
...
msize_t mvs_request_queue_manager_enqueue_request(MVSRequestQueueManager *req_manager, MVSGravesRequest *req) {
	...
	mResult_t res;
	if ((res = mvs_dynamic_lqueue_enqueue(req_manager->request_queue, &req)) != MRES_SUCCESS) {
        ret = 1;
	} else {
...
MVSGravesRequest* mvs_request_queue_manager_dequeue_request(MVSRequestQueueManager* req_manager) {
	...
	if (mvs_dynamic_lqueue_dequeue(req_manager->request_queue, &req) != MRES_SUCCESS)
	   return NULL;
	return req;	
}
