...
mbool_t mvs_rlist_reader_init(MVSRlistReader *r, mstr_t file_path) {
  ...
  if (mvs_file_open(r->file, file_path, MVS_FILE_MODE_READ_WRITE) != MRES_SUCCESS) {
    fprintf(stderr, "Failed to open RLIST file: PATH=%s. Check if it exists and not a directory\n", file_path);
    return mfalse;
  }
  msize_t len, tmp;
