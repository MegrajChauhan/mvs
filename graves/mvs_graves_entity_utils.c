...
if (local_ent_list_en && !can_spawn_entity)
				 return mfalse;
		if ((conf & MVS_CONF_ENTITY_ENTITY_TRACKING_LIM) && (!can_spawn_entity || (can_spawn_entity && !local_ent_list_en)))
				 return mfalse;
		return mtrue;
}
...
mbool_t mvs_graves_entity_utils_init_entity(MVSEntity *ent, EntityContext *ctx, msize_t EID, mEntity_t type, 
				     msize_t conf, msize_t props, mqword_t in_conf
	...
	else if (in_conf && entry->check_setup && (entry->check_setup(in_conf) != mtrue)) {
...
mbool_t mvs_graves_entity_utils_prepare_entity(MVSEntity *ent, APIRequestResponse *resp, msize_t local_ent_list_size_lim) {
	...
	if (conf & MVS_CONF_ENTITY_LOCAL_ENTITY_LIST_ENABLE) {
			mResult_t res;
			if ((res = mvs_dynamic_listl_create(&ent->entity_local_list, lim, sizeof(MVSEntity*))) != MRES_SUCCESS) {
