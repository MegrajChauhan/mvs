...
mResult_t mvs_static_list_create(MVSStaticList **lst, msize_t cap,
  ...
  *lst = (MVSStaticList *)malloc(sizeof(MVSStaticList));
  ...
  (*lst)->buf = (mbptr_t)calloc(cap, elem_len);
  if (!(*lst)->buf) {
    free(*lst);
    return MRES_SYS_FAILURE;
  }
...
mResult_t mvs_static_list_push(MVSStaticList *lst, mptr_t elem) {
  ...
  if (lst->curr_ind >= lst->cap)
    return MRES_CONTAINER_FULL;

  memcpy((mptr_t)(lst->buf + (lst->curr_ind * lst->elem_len)), elem,
         lst->elem_len);
  lst->curr_ind++;
  return MRES_SUCCESS;
}
...
mResult_t mvs_static_list_pop(MVSStaticList *lst, mptr_t elem) {
  ...
  lst->curr_ind--;
  if (elem)
    memcpy(elem, (mptr_t)(lst->buf + (lst->curr_ind * lst->elem_len)),
           lst->elem_len);
  return MRES_SUCCESS;
}
...
mResult_t mvs_static_list_resize(MVSStaticList *lst, msize_t resize_factor) {
  ...
  if (resize_factor == 1)
    return MRES_SUCCESS;
  mbptr_t new_buf = (mbptr_t)malloc(lst->elem_len * lst->cap * resize_factor);
  if (!new_buf)
    return MRES_SYS_FAILURE;
...
mResult_t mvs_dynamic_listl_create(MVSDynamicListLinear **lst, msize_t cap,
  ...
  *lst = (MVSDynamicListLinear *)malloc(sizeof(MVSDynamicListLinear));
  ...
  (*lst)->buf = (mbptr_t)malloc(cap * elem_len);
  if (!(*lst)->buf) {
    free(*lst);
    return MRES_SYS_FAILURE;
  }
...
mResult_t mvs_dynamic_listl_push(MVSDynamicListLinear *lst, mptr_t elem) {
  ...
  if (lst->curr_ind >= lst->cap) {
    mResult_t res;
    if ((res = mvs_dynamic_listl_resize(lst, lst->resize_factor)) !=
        MRES_SUCCESS)
      return res;
  }
  memcpy((mptr_t)(lst->buf + (lst->curr_ind * lst->elem_len)), elem,
...
mResult_t mvs_dynamic_listll_push(MVSDynamicListLinkedList *lst, mptr_t elem) {
  if (!lst || !elem)
    return MRES_INVALID_ARGS;
  MVSDynamicListLinkedListNode *new_node = mvs_dynamic_listll_get_new_node(lst);
  if (!new_node)
    return MRES_SYS_FAILURE;
  memcpy(new_node->data, elem, lst->elem_len);
  if (!lst->data_count) {
    new_node->next = NULL;
    new_node->prev = NULL;
    lst->chain = new_node;
