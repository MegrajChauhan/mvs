...
/*
 * conf will configure the interface whereas flag defines the behavior of the instance
 */
 ...
 * The owner should using the following call to map the size needed by the file if file 
...
/*
 * The flags here are owner-specified. MVS_MMAPPED_FLAG_EXEC doesn't do anything(yet)
 * If file_path is NULL than the mapping can be configured according to the flags.
 * But if file_path is NULL, than offset refers to the size of the mapping and it should't
 * be larger than the original mapping.
 * If mvs_mapped_memory_map hasn't been called yet and, thus, no mapping created, the 
 * function will fail immediately.
 * The file is opened by default in such a way that it doesn't conflict with the 
 * flags provided in 'flags'.
 * If flags = 0 than the default mapping is set: for backing file- RW+P(Copy-On Write), for mapped memory- RW+P
 */
