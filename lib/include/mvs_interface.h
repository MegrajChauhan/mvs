...
struct MVSInterface {
  ...
  atm_mqword_t config; // This configuration cannot be changed once set during intialization
  atm_mqword_t flags;  // These are mutable flags to manipulate the interface's behavior
  ...
  union {
    ...
    struct {
    	...
    	struct {
    		...
    		msize_t shared: 1;  // The updates on the address space is shared with other processes sharing the same memory region
            msize_t private:1;   // Opposite of shared
            msize_t sync: 1;  // This causes the changes made on the mapped memory to be immediately reflected on the file(the operation is synchronous)
...
/*
 * This just creates the interface. It doesn't have any type just yet and can be turned
 * into any type as required.
 */
mResult_t mvs_interface_create(MVSInterface **interface);

/* if conf = 0 than the interface will be initialized with default configurations:
 * SHAREABLE = false
 */
...
/*
 * This initializes the interface with a type. The interface hasn't necessarily been
 * initialized yet but it has been designated a type for now. The initialization may
 * follow later on. If the initialization fails later on than the interface is made
 * 'free' immediately.
 */
...
/*
 * Sharing of the interface happens by simply sharing the same interface. A new instance 
 * isn't allocated and we keep track of the owners using the owner_count field.
 * This is acceptable as every entity using the interface is a part of the same process
 */
