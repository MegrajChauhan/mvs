...
_MVS_ATTR_INTERNAL_ MVSLogger *_logger; // Lets keep a local copy instead of passing it as
                                        // argument each time
_MVS_ATTR_INTERNAL_ mstr_t msg_type[] = {
	"Note", "Warning", "Error", "Debug"
};
...
_MVS_ATTR_INTERNAL_ void mvs_logger_log_all() {
    ...
    while (!_MVS_HYBRID_CONCURRENCY_MODEL_QUEUE_CHECK_ISEMPTY_(_logger->queue)) {
   	  if ((res = mvs_hybrid_concurrency_model_queue_dequeue(_logger->queue, &entry)) != MRES_SUCCESS) {
...
mResult_t mvs_logger_init(mLogLvl_t allowed) {
    ...
    mResult_t res = mvs_hybrid_concurrency_model_queue_create(&_logger->queue, 100/*100 logs for now*/, sizeof(MVSLogEntry));
	if (res != MRES_SUCCESS) {
		free(_logger);
		return res;
	}
